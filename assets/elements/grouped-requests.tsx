...
export default class GroupedRequests extends AbstractCustomElement {
    Element({ requests, withoutCollectionName }: { requests: string, withoutCollectionName?: string }) {
        ...
        const deleteSelectedRequests = async () => {
            ...
            const ids = Array.from($inputs).map((input) => Number((input as HTMLInputElement).dataset.requestId!));
            ...
            if (res.ok) {
                const updatedRequests = Object.entries(groupedRequests).reduce((acc, [date, requests]) => {
                    const filteredRequests = requests.filter((request) => !ids.includes(request.id));
                    if (filteredRequests.length > 0) {
                        acc[date] = filteredRequests;
                    }
                    return acc;
                ...
                console.error("Error deleting requests:", res.statusText);
            }
        ...
        return (
            <div>
                        ...
                        <div className="w-full" key={date}>
                            ...
                            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {requests.map((request) => (
                                    <div key={`request-${request.id}`} className="flex gap-x-4 items-center w-full">
                                        <input data-request-id={request.id} type="checkbox"
                                        ...
                                        >
                                            <div className="flex flex-col gap-2 max-w-full">
                                                ...
                                                {(withoutCollectionName !== "true") && request.collection && (
                                                    <div>
                ...
                {isSelecting &&
                    <button onClick={deleteSelectedRequests} className="button-primary w-full mt-6 gap-x-2">
